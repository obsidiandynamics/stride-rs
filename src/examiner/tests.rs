...
#[test]
fn compress() {
    assert_eq!((vec![].into(), 10), Record::compress(vec![3, 9, 6], 10));
    assert_eq!((vec![6, 9].into(), 4), Record::compress(vec![9, 6, 3], 4));
    assert_eq!((vec![6, 9].into(), 3), Record::compress(vec![3, 6, 9], 1));
}